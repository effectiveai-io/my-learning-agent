...
| 무엇 | 언제 | 누가 트리거 |
|---|---|---|
| **`/reflect` 호출** (인출·메타인지 1줄 생성) | 본인 리듬에 맞춰. PR 한 편 끝낸 뒤·자기 전·일요일 오후. 시간 제약 X | 본인 또는 cron |
| **뉴스레터 발송** (어제 outbox를 메일·디스코드로) | 매일 아침 7시 — 단순 알림 | 스케줄러 |

핵심은 — `/reflect`는 *학습 행위*라 본인 리듬에 따라, 뉴스레터는 *알림*이라 아침 7시. 둘이 같은 시간일 필요 없습니다. 어제 자기 전 `/reflect`로 만든 outbox 글이 오늘 아침 7시 뉴스레터로 본인에게 도착하는 식.
