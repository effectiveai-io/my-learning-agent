...
- **사람용 영역 `outbox/`** — 내가 직접 쓰는 자리. ACE 분류(Atlas·Effort·Calendar)와 의미 있는 파일명으로. 쓰는 동안 학습이 일어나는 자리라, 사람이 보고 찾고 다시 꺼내 쓰기 좋은 형식이 우선.
- **AI용 영역 `ai-wiki/`** — LLM이 읽는 자리. 시간순 평문 `YYYY-MM-DD-주제.md` 파일이 한 폴더에 평면으로. LLM이 컨텍스트로 한꺼번에 끌어와 읽기 좋은 형식이 우선.
