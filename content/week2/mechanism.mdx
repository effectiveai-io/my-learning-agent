...
답을 미리 한 줄로 적어두면 이렇습니다 — **사람에게 학습이 잘 일어나는 방식과 LLM에게 학습이 잘 일어나는 형식은 실제로 다르다. 그래서 한 저장소 안에 두 영역을 둔다. 사람은 `outbox/`에 쓰면서 학습하고, `ai-wiki/`는 LLM이 읽기 좋게 자동으로 채워진다.** 오늘의 한 줄 목표가 "내 Outbox에 글 1개 쌓는다"인 이유도 여기 있습니다.
...
- **AI용 영역 `ai-wiki/`** — `/reflect`가 매번 정리해 쓰는 자리. 시간순 평문 `YYYY-MM-DD-주제.md` 파일이 한 폴더에 평면으로. LLM이 컨텍스트로 한꺼번에 끌어와 읽기 좋은 형식이 우선.

두 영역 사이는 **자동 동기화**가 책임집니다. 사람은 `outbox/`에만 글을 쓰고, `/reflect` 같은 커맨드가 그 글을 `ai-wiki/`로 복사·정리합니다. 사람에게 학습이 잘 일어나는 방식과 LLM에게 학습이 잘 일어나는 형식이 실제로 다르다는 것을 인정하고, 같은 글을 두 가지 view로 나눈 셈입니다.

<Callout type="note">
실제 starter는 [imakerjun/my-brain-v1](https://github.com/imakerjun/my-brain-v1)이고, 거기엔 운영 보조 폴더 두 개(`inbox/` · `ai-output/`)가 더 있습니다 — raw 자료와 AI 산출물을 각각 따로 격리. 두 영역 골격은 그대로, [3. 셋팅](/week2/setup)에서 풉니다.
</Callout>
...
flowchart LR
    Me(["👤 나"])
    Out["📤 outbox/<br/>(사람용 · 자유 분류)<br/>파일명 = 주장"]
    AI["📚 ai-wiki/<br/>(AI용 · LLM 친화)<br/>YYYY-MM-DD-주제.md"]
    LLM(["🤖 LLM"])

    Me -- "쓴다<br/>(학습)" --> Out
    Out -- "자동 동기화<br/>(/reflect)" --> AI
    AI -- "읽는다<br/>(컨텍스트)" --> LLM
...
첫째, **사람은 `outbox/`만 신경쓰면 됩니다.** 본인에게 맞는 분류와 의미 있는 파일명으로 글을 쓰는 자리. 시간순 prefix 같은 운영 정보는 신경 안 써도 됩니다.

둘째, **자동 동기화가 `ai-wiki/`를 채웁니다.** `/reflect` 커맨드가 `outbox/`의 글을 읽어 시간 prefix로 `ai-wiki/`에 복제·정리. 사람은 한 번도 `ai-wiki/`를 직접 만지지 않습니다.

셋째, **raw는 안 건드리고, wiki는 호출 때마다 새로 쓴다.** 안드레 카파시 LLM wiki의 본질입니다. `outbox/`(raw)는 사람만 추가, 사람도 LLM도 *수정하지 않는* append-only 영역. `ai-wiki/`(wiki)는 `/reflect`가 호출될 때마다 새로 정리해 쓰는 영역. 사람은 한 번도 `ai-wiki/`를 직접 만지지 않습니다.

<Callout type="info">
**`ai-wiki/`는 `/reflect`가 호출될 때마다 새로 정리하는 LLM 작업 영역**입니다(카파시 LLM wiki 본질). raw인 `outbox/`는 사람만 추가하고 사람도 LLM도 수정하지 않는 append-only로 남겨두고, `ai-wiki/`는 호출 때마다 새로 채워집니다. 그리고 LLM이 만들어준 답을 그대로 `outbox/`에 붙이면 나는 받아 적은 셈이라 학습이 일어나지 않습니다. LLM 응답은 한 줄씩 내가 검토해서 채택할지 정하고, 채택한다면 내 손으로 `outbox/`에 옮겨 적는 자료일 뿐.
</Callout>
...
6원칙은 **사람이 다시 읽고 학습하기 위한 글 한 편의 형식**입니다(분류 폴더는 본인 자유). ai-wiki로 복제될 때 LLM이 읽기 좋은 형태로 자동 변환됩니다.
...
6원칙대로 쓴 1단락이면 충분합니다. 그 1글이 다음 `/reflect`의 첫 입력이 됩니다.
</Callout>
