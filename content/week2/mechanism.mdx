...
- **사람용 영역 `outbox/`** — 내가 직접 쓰는 폴더입니다. **본인에게 맞는 분류**와 의미 있는 파일명으로(ACE·PARA·날짜·주제 등 자유) 채워나갑니다. 쓰는 동안 학습이 일어나기 때문에, 사람이 보고 찾고 다시 꺼내 쓰기 좋은 형식을 우선합니다.
- **AI용 영역 `.ai-wiki/`** — `/돌아보기`(사람-소화 글의 `outbox/` 복제)와 `/수집`(`.inbox/` raw 자료의 미러)이 함께 채워줍니다. 시간순 평문 `YYYY-MM-DD-주제.md` 파일이 한 폴더에 평면으로 쌓이고, 모든 파일에 frontmatter `type`이 박혀 raw인지 사람-소화인지 한눈에 구분됩니다. LLM이 컨텍스트로 한꺼번에 끌어와 읽기 좋은 형식을 우선합니다.
